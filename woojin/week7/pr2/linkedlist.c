...
void print_list(node_t* hdr) {
    node_t* p = hdr;
    while (p != NULL) {
        printf("%3dX^%d ->", p->data.coef, p->data.exp);
        p = p->next;
    }
}
...
node_t* find_prev(node_t* hdr, elem_t e) {
    node_t* p = hdr;
    if (p == NULL) return p;
    while (p->next != NULL && p->next->data.coef != e.coef && p->next->data.exp != e.exp) {
        p = p->next;
    }
...
node_t* find(node_t* hdr, elem_t e) {
    node_t* p = hdr;
    while (p != NULL && p->data.coef == e.coef && p->data.exp == e.exp)
        p = p->next;
    return p;
}
...
node_t* delete_val(node_t* hdr, elem_t e) {
    ...
    if (hdr->data.coef == e.coef && hdr->data.exp == e.exp) { // 헤더면… 헤더를 삭제
