...
void input_list();
void print_list();
void add_list(elem_t);
void insert_list(int, elem_t);
elem_t delete_list(int);
elem_t update_list(int, elem_t);
...
int main()
{
    printf("좌표의 개수를 입력하시오 : ");
    scanf("%d", &size);
    printf("%d개의 좌표 입력하시오 : ", size);
    input_list();
    run_menu();
...
elem_t update_list(int pos, elem_t val)
{
    elem_t result = values[pos];
    values[pos] = val;
    printf("%d 번째 값을 %s로 변경합니다. ", pos, str(val));
    printf("이전값 = %s\n", str(result));
    return result;
}